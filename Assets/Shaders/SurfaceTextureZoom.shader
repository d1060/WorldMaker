Shader "Noise/PlanetarySurfaceTextureZoom"
{
    Properties
    {
        ...
        _ZoomTextureWidth("Zoom Texture Width", Float) = 2048
        _ZoomTextureHeight("Zoom Texture Height", Float) = 2048
        ...
        _CenterU("Center U", Float) = 0.5
        _CenterV("Center V", Float) = 0.5
        _BoundaryU("Boundary V", Float) = 0.5
        _BoundaryV("Boundary V", Float) = 0.5
    }
    ...
    SubShader
    {
        ...
        #define LONGITUDE_STEP 2.44140E-4f
        #define LATITUDE_STEP 4.88281E-4f
        #define PI 3.14159265359
        #define TWO_PI 6.28318530718

        float _TextureWidth;
        float _ZoomTextureWidth;
        float _ZoomTextureHeight;
        ...
        float2 uvAdd(float3 sphereCoordinates, float2 uvDelta, float3 up)
        {
            float3 rotationX = RotateAboutAxis(sphereCoordinates, up, uvDelta.x * LATITUDE_STEP);

            float3 right = cross(rotationX, up);

            float3 rotated = RotateAboutAxis(rotationX, right, uvDelta.y * LATITUDE_STEP);
        ...
        {
            float2 originalUV = IN.uv_HeightMap;
            if (originalUV.x > 2) originalUV.x -= 2;
            else if (originalUV.x > 1) originalUV.x -= 1;

            float Ustep = (0.5 - originalUV.x) * _BoundaryU;
            float Vstep = (originalUV.y - 0.5) * _BoundaryV;
            float Uangle = Ustep * 2 * PI;
            float Vangle = Vstep * PI;

            float3 position = UvToSphere(float2(_CenterU, _CenterV));
            ...
            float3 sphereCoordinates = RotateAboutAxis(rotationX, rightVector, Vangle);

            float2 uv = SphereToUv(sphereCoordinates);

            float height = 0;
            if (_IsZoomHeightMapSet)
            {
                float4 c = tex2D(_ZoomHeightMap, originalUV);
            ...
            if (_IsZoomHeightMapSet)
            {
                uvLeft = originalUV + float2(-1 / _ZoomTextureWidth, 0);
                uvBottom = originalUV + float2(0, -1 / _ZoomTextureHeight);
                uvRight = originalUV + float2(1 / _ZoomTextureWidth, 0);
                uvTop = originalUV + float2(0, 1 / _ZoomTextureHeight);
            }
            else
            {
                float3 right;
                ...
                uvLeft = uvAdd(sphereCoordinates, float2(-1, 0), up);
                if (uvLeft.x < 0) uvLeft.x += 1;

                uvBottom = uvAdd(sphereCoordinates, float2(0, -1), up);
                if (uvBottom.y < 0) uvBottom.y = 0;

                uvRight = uvAdd(sphereCoordinates, float2(1, 0), up);
                if (uvRight.x > 1) uvRight.x -= 1;

                uvTop = uvAdd(sphereCoordinates, float2(0, 1), up);
            ...
            else // DrawType != 1, 2, 7
            {
                ...
                {
                    float prevLatitudeHeight;
                    float prevLongitudeHeight;

                    if (_IsZoomHeightMapSet)
                    {
                        float4 c = tex2D(_ZoomHeightMap, uvLeft);
                        prevLongitudeHeight = (c.r + c.g + c.b) / 3;

                        c = tex2D(_ZoomHeightMap, uvBottom);
                        prevLatitudeHeight = (c.r + c.g + c.b) / 3;
                    }
                    else
                    {
                        float4 c = tex2D(_HeightMap, uvLeft);
                        prevLongitudeHeight = (c.r + c.g + c.b) / 3;

                        c = tex2D(_HeightMap, uvBottom);
                        prevLatitudeHeight = (c.r + c.g + c.b) / 3;
                    }
                    ...
                    if (_DrawType == 4) // Drawing a Normal mask.
                    {
                        ...
                        float horizontalAngle = atan(horizontalDeltaHeight / (1 / (2 * _ZoomTextureWidth))) / PI;
                        float verticalAngle = atan(verticalDeltaHeight / (1 / _ZoomTextureHeight)) / PI;
