...
float sphereNoise(float2 coords, float3 offset, int seed, float multiplier, float octaves, float lacunarity, float persistence, float range, int ridged)
{
    ...
    {
        float noiseValue = simplexNoise(sphereCoords) / SQRT_3_4; // 3D Simplex Noise ranges from -Sqrt(3/4) to +Sqrt(3/4)
        noiseValue /= range;
        if (ridged)
        {
